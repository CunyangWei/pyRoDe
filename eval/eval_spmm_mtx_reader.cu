...
#define VALIDATE_RESULTS 1 // Set to 1 to enable result validation
...
int main(int argc, char **argv)
{
    ...
    int *device_RowOffsets = (int *)malloc(sizeof(int) * (m + 1));
    CHECK_CUDA_ERROR(cudaMemcpy(device_RowOffsets, c_sm.RowOffsets(), sizeof(int) * (m + 1), cudaMemcpyDeviceToHost));
    int *device_ColumnIndices = (int *)malloc(sizeof(int) * c_sm.Nonzeros());
    CHECK_CUDA_ERROR(cudaMemcpy(device_ColumnIndices, c_sm.ColumnIndices(), sizeof(int) * c_sm.Nonzeros(), cudaMemcpyDeviceToHost));
    float *device_Values = (float *)malloc(sizeof(float) * c_sm.Nonzeros());
    CHECK_CUDA_ERROR(cudaMemcpy(device_Values, c_sm.Values(), sizeof(float) * c_sm.Nonzeros(), cudaMemcpyDeviceToHost));

    SPC::SparseMatrix sm2(device_RowOffsets, device_ColumnIndices, device_Values, m, k, c_sm.Nonzeros(), SPC::SORTED, 1);
    sm2.RowDivide2Segment(SEG_LENGTH, 4, 32);
    SPC::CudaSparseMatrix<float> c_sm2(sm2);
    ...
    std::cout << "RoDeSpmm_n32" << std::endl;
    std::cout << "  n_segs: " << c_sm2.n_segs << std::endl;
    std::cout << "  n_segs_residue: " << c_sm2.n_segs_residue << std::endl;
    std::cout << "  columns: " << c_sm2.Columns() << std::endl;

    if (n < 128)
    {
        for (int i = 0; i < ITER; ++i)
            RoDeSpmm_n32(c_sm2.n_segs, c_sm2.n_segs_residue, c_sm2.Columns(), n,
                         c_sm2.Values(), c_sm2.ColumnIndices(), c_sm2.RowOffsets(),
                         c_sm2.seg_row_indices, c_sm2.seg_row_indices_residue, c_sm2.seg_st_offsets,
                         d_B, d_C2, stream1, stream2);
    }
    else
    {
        for (int i = 0; i < ITER; ++i)
            RoDeSpmm_n128(c_sm2.n_segs, c_sm2.n_segs_residue, c_sm2.Columns(), n,
                          c_sm2.Values(), c_sm2.ColumnIndices(), c_sm2.RowOffsets(),
                          c_sm2.seg_row_indices, c_sm2.seg_row_indices_residue, c_sm2.seg_st_offsets,
                          d_B, d_C2, stream1, stream2);
    }
    ...
    gflops = (double)ITER * (double)c_sm2.Nonzeros() * 2 * n / tot_ms / 1000000;
